...
const Profile = ({ doctor }) => {
  ...
  const updateProfileHandler = async (e) => {
    e.preventDefault();
    try {
      const res = await instance.put(`doctors/${doctor._id}`, formData, {
            ...
            <div key={index}>
              <div>
                ...
                <button
                  ...
                  className="bg-red-600 p-2 rounded-full text-white test-[18px] mt-2 mb-[30px] cursor-pointer"
                >
                  <AiOutlineDelete />
                </button>
              </div>
        ...
        <div className="mb-5 flex items-center gap-3">
          {formData.photo && (
            <figure className="w-[60px] h-[60px] rounded-full border-2 border-solid border-primaryColor flex items-center justify-center">
              {loading && <Loading />}
              {!loading && <img src={formData.photo} alt="" className="w-full rounded-full" />}
            </figure>
          )}
